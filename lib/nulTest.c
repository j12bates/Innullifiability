...
// This function is simplified and optimized for length-3 sets. It uses
// only six checks and two operators since the pairs of arithmetic
...
// takes sets of positive integers with no double-values.
...
// function, and it performs every single possible operation on the set,
// reducing it, before recursing and checking again as though that were
// a new set. The initial reduction space gives the M-range that this
// 'new set' must fit in for it to go through the recursion. If it finds
// that the set is nullifiable at any point, that means the set was
// always nullifiable. A set is only innullifiable if the test always
// returns that result after every operation. This function only works
// on sets that are size-3 or larger, containing positive values only,
// and who have no double-values. The maximum M-value can be set to zero
// to indicate no upper bound. Also, this does not check the cases where
// there was wrap-around, so technically any innullifiable outputs have
// to undergo a check that can handle large integers just to be sure.
        ...
        unsigned long minm, unsigned long maxm)
{
    ...
    // Try every arithmetic operation we can on every pair we can, and
    // pass in that new 'set' to ourselves. If we can show
    // nullifiability at any point, that carries on
        ...
        for (size_t pairB = pairA + 1; pairB < size; pairB++)
    {
        ...
        // Don't bother with the product if it'll wrap around
        if (a <= UINT_MAX / b) replacements[3] = a * b;

        // If any of these equal any of the existing values, that's a
        // double-value and a zero
        for (size_t i = 0; i < 4; i++)
            for (size_t index = 1; index < size - 1; index++)
                if (replacements[i] == newSet[index])
                    return 0;
